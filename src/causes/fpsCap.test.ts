...
import { causeFpsCap } from './fpsCap';
...
const cappedWa = makeWindowAnalysis([makeWindow(0, { limiter: 'capped', metrics: { fpsAvg: 120 } })]);

describe('causeFpsCap', () => {
  it('suggests undervolting when FPS is capped and there is thermal/power headroom', () => {
    ...
    const log = makeLog({ sensors, fps: { source: 'displayed', capped: true, capValue: 60 } });
    const events = causeFpsCap(log, statsFor(sensors), cappedWa);
  ...
  it('emits nothing when FPS is not capped', () => {
    ...
    expect(causeFpsCap(log, statsFor(sensors), cappedWa)).toEqual([]);
  });
