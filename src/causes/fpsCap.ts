...
export function causeFpsCap(
  ...
  if (hasHeadroom) {
    ...
    return [makeEvent({ type: 'fps-cap', severity: 'info', sentence, fix: HEADROOM_FIX, sampleCount: fps.clean.length, evidence })];
  }

  // Capped but the GPU is already working hard — note the cap without the headroom claim.
  const sentence = `FPS is capped at ~${cap}.`;
  return [makeEvent({ type: 'fps-cap', severity: 'info', sentence, sampleCount: fps.clean.length, evidence })];
}
